...
| 모델 선택 | Qwen, OpenAI, Gemini, Claude, DeepSeek와 Ollama / LM Studio 로컬 모델을 지원합니다(API 키는 직접 준비). |
...
| 방식 | 설정 |
| --- | --- |
| 클라우드 모델 | 설정에서 Qwen, OpenAI, Gemini, Claude 또는 DeepSeek를 선택하고 API 키를 입력합니다. 호환 엔드포인트는 Base URL을 직접 지정할 수 있습니다. |
