...
## ☁️ Recommended LLM: Atlas Cloud (OpenAI-compatible)

<p align="center">
  <a href="https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=autoclip">
    <img src="docs/atlas-cloud-logo.png" alt="Atlas Cloud" width="200">
  </a>
</p>

> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=autoclip)** is a full-modal, OpenAI-compatible AI inference platform — plug it in as a drop-in backend for AutoClip's content analysis and highlight extraction to access DeepSeek, Qwen, GLM, Kimi, MiniMax and more through a single API, no multi-vendor setup needed. Budget-friendly: [coding plan](https://www.atlascloud.ai/console/coding-plan).

AutoClip ships with a built-in `atlascloud` provider. On the **Settings** page, choose **Atlas Cloud** and paste your API Key; the default model is `deepseek-ai/deepseek-v4-pro`.
...
> `deepseek-ai/deepseek-v4-pro` is a reasoning model — give it enough `max_tokens` (>= 512), otherwise tokens may be spent on the chain-of-thought first, yielding `finish_reason=length` with empty `content`. AutoClip's `AtlasCloudProvider` already applies this fallback.

<details>
<summary>All Atlas Cloud chat models (59, synced with /zh/models/list/llm)</summary>
