      ...
      <Modal
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            {projectId && <Btn size="sm" onClick={() => { setShowPlayer(false); setShowExport(true) }}>{t("发布导出")}</Btn>}
        ...
        footer={
          <div className="right" style={{ marginLeft: 'auto' }}>
            {projectId && !exporting && (
              <Btn size="sm" onClick={() => {
                setShowExport(false)
                const q = new URLSearchParams({ preset, subtitles: burnSub ? '1' : '0', titleCard: titleCard ? '1' : '0' })
                navigate(`/project/${projectId}/publish/${clip.id}?${q}`)
              }}>{t("发到海外平台")}</Btn>
            )}
