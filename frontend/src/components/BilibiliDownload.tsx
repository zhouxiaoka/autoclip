...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const parseVideoInfo = async () => {
    ...
    if (!videoType) {
      setError(t("请输入正确的B站或YouTube视频链接"))
      return
    }
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (task.status === 'completed') {
          ...
          setPollingInterval(null)
          setDownloading(false)
          message.error(t("下载失败: {{value1}}", { value1: task.error_message || '未知错误' }))
          resetForm()
        }
      } catch (error) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.project_id) {
        ...
        // 显示统一的成功提示
        const platformName = videoType === 'bilibili' ? t("B站") : 'YouTube'
        message.success(t("{{value1}}项目创建成功，正在后台下载中，您可以继续添加其他项目", { value1: platformName }))
      ...
      setDownloading(false)
      const errorMessage = error.response?.data?.detail || error.message || t("创建下载任务失败")
  ...
  return (
    <div style={{
        ...
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          ...
          {videoInfo && (
            <div style={{
              ...
              <Text style={{ color: '#667eea', fontWeight: 600, fontSize: '16px', display: 'block', marginBottom: '8px' }}>{t("视频信息解析成功")}</Text>
              <Text style={{ color: '#ffffff', fontSize: '14px', display: 'block' }}>
                {videoInfo.title}
              </Text>
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                {getVideoType(url) === 'bilibili' ? t("UP主") : t("频道")}: {videoInfo.uploader || t("未知")}{t("• 时长:")}{videoInfo.duration ? `${Math.floor(videoInfo.duration / 60)}:${String(Math.floor(videoInfo.duration % 60)).padStart(2, '0')}` : t("未知")}
              </Text>
            </div>
          )}
          ...
          {videoInfo && (
            <>
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>{t("项目名称（可选）")}</Text>
                <Input
                  placeholder={t("留空将使用视频标题作为项目名称")}
                  value={projectName}
              ...
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>{t("视频分类")}</Text>
                {loadingCategories ? (
                  <Spin size="small" />
                ) : (
                  <div style={{
      ...
      {currentTask && (
        ...
        >
          <div style={{ marginBottom: '16px' }}>
            <Text style={{ color: '#ffffff', fontWeight: 600, fontSize: '18px' }}>{t("导入进度")}</Text>
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <Text style={{ color: 'var(--ac-sub)', fontSize: '14px' }}>{t("状态:")}{currentTask.status}</Text>
          ...
          {currentTask.error_message && (
            <div style={{ 
              ...
              <Text style={{ color: '#ff4d4f', fontSize: '14px' }}>{t("错误:")}{currentTask.error_message}</Text>
            </div>
          )}
        </Card>
      )}
