...
export default function ExportHistory({ projectId, jobs, open, onClose }: { projectId: string; jobs: RenderJob[]; open: boolean; onClose: () => void }) {
  ...
  return <Dialog open={open} onClose={onClose} title={t('导出记录')} description={t('保留每次导出的成片与草稿版本。')}>
    ...
    {jobs.map(job => <div key={job.job_id} className="studio-history-row"><b>{job.title}</b><span className="studio-muted">V{job.revision} · {new Date(job.created_at).toLocaleString(getLocale())}</span>
      {job.status === 'completed' ? <><a className="ac-btn" href={studioApi.video(projectId, job.job_id, true)} download onClick={studioDownloadRequested}>{t('下载成片')}</a><Link className="studio-link" to={`/project/${projectId}/publish/studio-${job.job_id}`}>{t('发布这版成片')}</Link>{job.result?.warnings?.map(w => <p className="studio-muted" key={w}>{w}</p>)}</> : job.status === 'failed' ? <p className="studio-error">{job.error}</p> : <><ProgressLine percent={job.percent} /><span className="studio-muted">{t(job.status === 'queued' ? '排队中' : '正在渲染')} · {job.percent}%</span></>}
    </div>)}
  </Dialog>
}
