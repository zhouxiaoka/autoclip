...
export default function ImportPreferences({value, onChange, hideGoal=false}: {value: ImportOptions; onChange: (value: ImportOptions) => void; hideGoal?:boolean}) {
  ...
  return <div className="studio-fields">
    ...
    <label className="studio-field">期望时长<select value={value.duration ?? ''} onChange={e=>patch({duration:e.target.value?Number(e.target.value):null})}><option value="">AI 根据内容匹配</option>{[15,30,60,90,120].map(n=><option value={n} key={n}>约 {n} 秒</option>)}</select>{value.goal==='content'&&<small>内容切片按完整语义选段，具体起止可在编辑器调整。</small>}</label>
