...
export default function CreativeImport({ onImported }: { onImported: () => Promise<void> }) {
  ...
  return <section className="ac-creative-import">
    ...
    <div className="studio-import-box">
      <div className="studio-row"><Segmented ariaLabel="导入来源" value={source} onChange={v=>!busy&&setSource(v)} options={[{value:'link',label:t("链接导入")},{value:'file',label:t("文件导入")}]}/></div>
    ...
    <Dialog open={preferences} onClose={()=>setPreferences(false)} title="制作偏好" description="默认由 AI 匹配。你指定的选项优先，生成后也可以修改。" footer={<div className="studio-actions"><Btn onClick={()=>{setOptions({...defaultImportOptions,instruction:options.instruction});setSubtitle(null);setBrowser('')}}>恢复自动</Btn><Btn variant="cta" onClick={()=>setPreferences(false)}>{t("完成")}</Btn></div>}>
      <ImportPreferences value={options} onChange={setOptions}/>
      <label className="studio-field">已有字幕（选填）<input type="file" accept=".srt" aria-label="已有字幕" onChange={e=>setSubtitle(e.target.files?.[0] || null)}/>{subtitle&&<small>{subtitle.name}<button className="studio-link" onClick={()=>setSubtitle(null)}>{t("移除")}</button></small>}</label>
