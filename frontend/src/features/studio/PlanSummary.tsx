  ...
  return <>
    <div className={`studio-plan-summary ${awaiting?'studio-plan-confirm':''}`}>
      ...
      {awaiting && plan ? <>
        ...
        <div className="studio-row studio-confirm-footer"><span className="studio-muted">{selected.length?t('将制作 {{count}} 类内容', { count: selected.length }):t("至少选择一种制作类型")}</span><Btn variant="cta" disabled={!selected.length||busy} loading={busy} onClick={start}>{t("确认并开始制作")}</Btn></div>
