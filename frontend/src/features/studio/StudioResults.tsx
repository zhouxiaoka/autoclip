...
export default function StudioResults({ project, children, onCreateCollection, onReload }: { project: Project; children: ReactNode; onCreateCollection: () => void; onReload: () => void }) {
  ...
  return <>
    <Section title="你的成片" count={workspace.drafts.length+(project.clips?.length||0)+(project.collections?.length||0)} description={visual?'从真实画面中寻找精彩，选喜欢的成片继续修改或导出。':'切片与合集都在这里，可以直接下载，也可以另存为成片草稿。'} right={<div className="studio-actions"><Btn size="sm" onClick={()=>setHistory(true)}>导出记录</Btn>{visual?<Btn size="sm" disabled={busy!==''||workspace.analysis?.status==='running'} loading={busy==='analyze'} onClick={()=>act('analyze',async()=>{await studioApi.analyze(project.id);onReload()})}>{workspace.analysis?.status==='failed'?'重试分析':'再生成'}</Btn>:<Btn size="sm" onClick={onCreateCollection}>新建合集</Btn>}</div>}>
    ...
    <Dialog open={!!exporting} title="导出成片" onClose={()=>!busy&&setExporting(null)} description="按当前草稿设置渲染，完成后在导出记录下载。" footer={<div className="studio-actions"><Btn disabled={!!busy} onClick={()=>setExporting(null)}>关闭</Btn><Btn variant="cta" disabled={!!busy} loading={busy==='export'} onClick={()=>exporting&&act('export',async()=>{await studioApi.export(project.id,exporting.id);setExporting(null);setHistory(true)})}>确认导出</Btn></div>}><p>{exporting?.title}</p><p className="studio-muted">{exporting?.aspect==='portrait'?'9:16 竖屏':exporting?.aspect==='landscape'?'16:9 横屏':'原画幅'} · MP4 · 30 fps</p>{actionError&&<p className="studio-error">{actionError}</p>}</Dialog>
  </>
}
