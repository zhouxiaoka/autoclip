...
export default function StudioResults({ project, children, onCreateCollection, onReload }: { project: Project; children: ReactNode; onCreateCollection: () => void; onReload: () => void }) {
  ...
  return <>
    ...
    {!(workspace.analysis?.status==='awaiting_confirmation' && !workspace.drafts.length && !project.clips?.length && !project.collections?.length) && <Section title="剪辑与成片" count={workspace.drafts.length+(project.clips?.length||0)+(project.collections?.length||0)} description={visual?'候选已整理为可编辑草稿；导出完成后，才能下载带包装的视频。':'切片与合集都在这里，可以直接下载，也可以另存为成片草稿。'} right={<div className="studio-actions"><Btn size="sm" onClick={()=>setHistory(true)}>导出记录</Btn>{!managed&&<Btn size="sm" onClick={onCreateCollection}>新建合集</Btn>}</div>}>
