...
function Editor({ projectId, draftId }: { projectId: string; draftId: string }) {
  ...
  const artworkStyle = ['comic','neon','arena','editorial'].includes(draft?.title_style || '')
  ...
  return <div className="ac-page studio-editor-page">
    ...
    <fieldset disabled={!!busy} className="studio-fieldset">
      ...
      <aside className="studio-edit-panel"><h2>改到满意，就导出。</h2><Btn size="sm" onClick={() => patch(portraitDesign(draft))}>应用竖屏推荐</Btn><label className="studio-field">成片名称<input maxLength={200} value={draft.title} onChange={e => patch({title:e.target.value})} /></label><label className="studio-field">开头文字<textarea maxLength={120} value={draft.hook} placeholder="可留空；在开头约 4 秒显示" onChange={e => patch({hook:e.target.value})} /></label>
        <div className="studio-field"><span>标题模板</span><div className="studio-template-options">{([{value:'comic',label:'漫画冲击'},{value:'neon',label:'荧光挑战'},{value:'arena',label:'竞技斜切'},{value:'editorial',label:'极简大字'},{value:'impact',label:'高能大字'},{value:'card',label:'简洁标题卡'},{value:'plain',label:'基础文字'}] as const).map(t=><button type="button" key={t.value} className={`studio-template studio-template--${t.value} ${['comic','neon','arena','editorial'].includes(t.value)?'studio-template--art':''}`} aria-pressed={(draft.title_style || 'plain') === t.value} onClick={()=>patch({title_style:t.value,title_template_version:2,title_accent:null})}>{['comic','neon','arena','editorial'].includes(t.value)&&<img src={studioApi.titleThumbnail(t.value)} alt=""/>}<span>{t.label}</span></button>)}</div></div>{artworkStyle && <details className="studio-details"><summary>调整文字样式</summary>{draft.title_style!=='editorial' && <label className="studio-field">样式版本<select aria-label="样式版本" value={draft.title_template_version??1} onChange={e=>patch({title_template_version:Number(e.target.value) as 1|2})}><option value={2}>加强版</option><option value={1}>原版</option></select></label>}<label className="studio-field">强调色<input type="color" aria-label="标题强调色" value={draft.title_accent || (draft.title_style==='comic'?'#ffe52d':draft.title_style==='neon'?'#ccff00':draft.title_style==='editorial'?'#ff4826':'#dfff00')} onChange={e=>patch({title_accent:e.target.value})}/></label><label className="studio-field">文字大小<input type="range" aria-label="文字大小" min=".75" max="1.2" step=".05" value={draft.title_scale??1} onChange={e=>patch({title_scale:Number(e.target.value)})}/></label><label className="studio-field">文字位置<input type="range" aria-label="文字位置" min=".06" max=".70" step=".01" value={draft.title_y??.12} onChange={e=>patch({title_y:Number(e.target.value)})}/></label><label><input type="checkbox" checked={draft.title_motion??true} onChange={e=>patch({title_motion:e.target.checked})}/> 开启入场动效</label><p className="studio-muted">预览使用实际文字图层。翻译与入场动效以渲染结果为准；支持手动换行。</p></details>}<label className="studio-field">输出文字语言<select value={draft.language} onChange={e => patch({language:e.target.value as Draft['language']})}>{languages.map(l => <option key={l.value} value={l.value}>{l.label}</option>)}</select></label>
