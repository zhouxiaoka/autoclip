...
function Editor({ projectId, draftId }: { projectId: string; draftId: string }) {
  ...
  return <div className="ac-page studio-editor-page">
    ...
    <fieldset disabled={!!busy} className="studio-fieldset">
      ...
      <aside className="studio-edit-panel"><h2>改到满意，就导出。</h2><Btn size="sm" onClick={() => patch(portraitDesign(draft))}>应用竖屏推荐</Btn><label className="studio-field">成片名称<input maxLength={200} value={draft.title} onChange={e => patch({title:e.target.value})} /></label><label className="studio-field">开头文字<textarea maxLength={120} value={draft.hook} placeholder="可留空；在首镜头最多显示 4 秒" onChange={e => patch({hook:e.target.value})} /></label>
        <div className="studio-field"><span>标题模板</span><small className="studio-muted">缩略图为设计参考，当前文案效果见预览。</small><div className="studio-template-options">{titlePresets.map(t=><button type="button" key={t.value} className={`studio-template studio-template--${t.value} ${isArtworkStyle(t.value)?'studio-template--art':''}`} aria-pressed={(draft.title_style || 'plain') === t.value} onClick={()=>patch({title_style:t.value,title_template_version:isArtworkStyle(t.value)?6:1,title_accent:null})}>{isArtworkStyle(t.value)&&<img src={titleDesignThumbnails[t.value]} alt="" width={360} height={240}/>}<span>{t.label}</span></button>)}</div></div>{artworkStyle && <details className="studio-details"><summary>调整文字样式</summary><label className="studio-field">样式版本<select aria-label="样式版本" value={draft.title_template_version??1} onChange={e=>patch({title_template_version:Number(e.target.value) as Draft['title_template_version']})}>{titleVersions(draft.title_style).map(v=><option key={v.value} value={v.value}>{v.label}</option>)}</select></label><label className="studio-field">强调色<input type="color" aria-label="标题强调色" value={draft.title_accent || (draft.title_style==='comic'?'#ffe52d':draft.title_style==='neon'?'#ccff00':draft.title_style==='editorial'?'#ff4826':draft.title_style==='pixel'?'#ed327c':draft.title_style==='frosted'?'#00e6dc':'#dfff00')} onChange={e=>patch({title_accent:e.target.value})}/></label><label className="studio-field">文字大小<input type="range" aria-label="文字大小" min=".75" max="1.2" step=".05" value={draft.title_scale??1} onChange={e=>patch({title_scale:Number(e.target.value)})}/></label><label className="studio-field">文字位置<input type="range" aria-label="文字位置" min=".06" max=".70" step=".01" value={draft.title_y??.12} onChange={e=>patch({title_y:Number(e.target.value)})}/></label>{!['pixel','frosted'].includes(draft.title_style||'') && <label><input type="checkbox" checked={draft.title_motion??true} onChange={e=>patch({title_motion:e.target.checked})}/> 开启入场动效</label>}<p className="studio-muted">预览使用实际文字图层。翻译与入场动效以渲染结果为准；支持手动换行。</p></details>}<label className="studio-field">输出文字语言<select value={draft.language} onChange={e => patch({language:e.target.value as Draft['language']})}>{languages.map(l => <option key={l.value} value={l.value}>{l.label}</option>)}</select></label>
    ...
    <Dialog open={showExport} onClose={()=>!busy && setShowExport(false)} title="导出成片" description="保存当前修改并渲染；已有输出会保留在导出记录。" footer={<div className="studio-actions"><Btn disabled={!!busy} onClick={()=>setShowExport(false)}>关闭</Btn>{previewUrl && <Btn onClick={()=>navigate(`/project/${projectId}/publish/studio-${currentJob!.job_id}`)}>发布这版成片</Btn>}{previewUrl ? <a className="ac-btn ac-btn--cta" href={studioApi.video(projectId,currentJob!.job_id,true)} download onClick={studioDownloadRequested}>下载成片</a> : <Btn variant="cta" loading={busy==='render'||!!active} disabled={!!active} onClick={render}>确认导出</Btn>}</div>}>
      ...
      <Row label="开头包装">{draft.hook ? titlePresets.find(t=>t.value===(draft.title_style||'plain'))?.label : '无开头文字'}</Row><Row label="原声">{draft.original_audio?'保留':'关闭'}</Row><Row label="字幕">{draft.subtitles?'烧录已有字幕':'关闭'}</Row>
