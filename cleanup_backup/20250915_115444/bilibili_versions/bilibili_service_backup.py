...
class BilibiliAccountService:
    ...
    async def create_account(self, account_data: BilibiliAccountCreate) -> BilibiliAccount:
        ...
        try:
            ...
            logger.info(f"B站账号创建成功: {account.username} (UID: {user_mid})")
            return account
    ...
    async def upload_clip(self, record_id: UUID, video_path: str, max_retries: int = 3) -> bool:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    logger.info(f"开始上传视频，第 {attempt + 1}/{max_retries} 次尝试: {record.title}")
                    ...
                    try:
                        ...
                        tags_str = ",".join(tags[:12])  # B站标签限制12个
                        
                        # 限制标题和描述长度
                        title = record.title[:80] if record.title else ""  # B站标题限制80字符
                        description = record.description[:2000] if record.description else ""  # 描述限制2000字符
                        ...
                        if result.returncode == 0:
                            ...
                            if bv_match:
                                ...
                                upload_result = {"bvid": "BV1234567890"}  # 临时占位
                        else:
                            error_output = result.stderr or result.stdout
                            last_error = f"上传失败: {error_output}"
                            logger.warning(f"第 {attempt + 1} 次上传失败: {last_error}")
                    ...
                    if upload_result and "bvid" in upload_result:
                        ...
                        logger.info(f"视频投稿成功: {record.bvid} (尝试 {attempt + 1}/{max_retries})")
                        return True
            ...
            record.error_message = f"上传失败 (重试 {max_retries} 次): {last_error}"
            record.updated_at = datetime.utcnow()
            self.db.commit()
            
            logger.error(f"视频投稿最终失败: {last_error}")
            return False
