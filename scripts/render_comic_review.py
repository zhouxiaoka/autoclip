"""Reproducible comparison of the design reference, v3 and comic v4."""
import argparse
import json
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='comic-v4-') as data:
        ...
        evidence=[]
        for key,hook in [('comic-v4','CAN YOU\nESCAPE?'),('comic-zh','你能逃出\n这里吗？')]:
            draft=Draft(id=key,title=key,hook=hook,title_style='comic',title_template_version=4,
        ...
        for suffix in ['.mp4','.jpg']:shutil.copy2(args.baseline/f'comic-v3{suffix}',out/f'comic-v3{suffix}')
