...
### 改进
- **设置页模型名单跟得上服务商**：通义千问 / OpenAI / Gemini / 硅基流动的下拉换成 2026 年常用型号（含 qwen3.8、gpt-5.6、gemini-2.5）。填写密钥后会向服务商拉取最新 `/models`，并与内置名单合并；拉不到时仍显示内置列表，可点刷新。自建 OpenAI 兼容地址同样会探测。
