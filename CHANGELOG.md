...
### 新增
- 新增可选的 TwelveLabs Pegasus 视频语义评分后端：设置 `SCORING_BACKEND=pegasus` 并提供 `TWELVELABS_API_KEY` 后，Step 3 精彩度评分会直接调用 Pegasus 视频理解模型对视频片段打分，替代仅基于字幕文本的启发式评分。默认行为不变，未配置时自动回退到字幕评分器。
