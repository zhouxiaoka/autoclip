...
2. 打开切片的 **发布导出**，选好画幅预设后点 **发到海外平台**。这一页只做几件事：现在发或选一个时间、勾选已连接的平台、可见范围默认「仅自己」。标题不填就用切片标题。
3. 现在发会先渲成片再等各平台结果。定时会在成片上传之后交给 Upload-Post，到点才发出，这时可以关掉应用。项目页的 **发布** 能看到这些记录；还没发出的排期可以取消。渲成片的过程中不要关窗口。
