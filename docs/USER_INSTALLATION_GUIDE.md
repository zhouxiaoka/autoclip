...
1. **先配置模型。** 打开“设置”，选择 Qwen、OpenAI、Gemini、DeepSeek、Doubao Seed、Kimi、GLM 或本地模型。云端服务填写自己的 API Key 和模型名，点击测试连接，再保存；兼容接口可配置 Base URL。本地模型先在 Ollama / LM Studio 启动服务并加载模型。
