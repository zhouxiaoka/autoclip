...
后续清理说明：未接入的历史文件已在本分支删除；本文件保留改造前的界面快照。API 配置检查条目已依据 TypeScript 实际解析结果修正，不能仅凭同名 `.tsx` 文件推断行为。
...
| API 配置 | 原导入代码调用同名检查辅助函数，但解析到 `.ts` 的实现直接返回 true | 不能视为已经校验模型配置；未生效的 `.tsx` 检查不属于实际入口 |
