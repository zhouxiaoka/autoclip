...
1. **Configure a model.** Open Settings and select Qwen, OpenAI, Gemini, DeepSeek, Doubao Seed, Kimi, GLM, or a local model. For cloud services, enter your own API key and model name, test the connection, and save. Compatible endpoints support a custom Base URL. For Ollama / LM Studio, start the model service and load a model first.
