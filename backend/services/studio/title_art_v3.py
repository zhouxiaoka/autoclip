"""Glossy ink, pixel arcade and genuine backdrop glass, version 3."""
from PIL import Image, ImageDraw
from backend.services.studio.title_materials import gloss_text, gradient_shape, pixel_artwork, frosted_layers
...
def artwork(text, style, w, h, scale=1, y=.12, accent=None):
    ...
    if style == 'pixel':
        return pixel_artwork(text, w, h, scale, y, accent)
    if style == 'frosted':
        return frosted_layers(text, w, h, scale, y, accent)[0]
    ...
    for i, (line, font, box, tw, th, n) in enumerate(zip(lines, fonts, boxes, widths, heights, sizes)):
        ...
        if style == 'comic':
            ...
            gloss_text(art, (px, py), line, font, color, strength=1)
        elif style == 'neon':
            if i == len(lines)-1:
                margin = pad*.38
                gradient_shape(art, [(px-margin, top-gap*.40),(px+tw+margin, top-gap*.40),(px+tw+margin, top+th+gap*.50),(px-margin, top+th+gap*.50)], color, .55)
                ...
                gloss_text(art, (px, py), line, font, '#f1f5ff', strength=.8)
        elif style == 'arena':
            skew = pad*.40
            ...
            gradient_shape(art, [(left+skew,yt),(right+skew,yt),(right-skew,yb),(left-skew,yb)], '#111f34', .4)
            d.line([(left+skew,yt),(right+skew,yt)],fill='#647588',width=max(1,round(w*.002)))
            gloss_text(art, (px, py), line, font, color if i == len(lines)-1 else '#f1f5ff', strength=1)
        else:
            if i == len(lines)-1:
            ...
            gloss_text(art, (px,py), line, font, '#fff9e9', strength=.5)
        top += th+gap
    if style == 'comic':
