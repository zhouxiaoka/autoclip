...
def render_draft(project_id, video, draft: Draft, job_id, progress):
    ...
    try:
        ...
        return {'title': draft.title, 'duration': _probe(output).get('duration'), 'width': w, 'height': h, 'warnings': warnings, 'cta': {k:v for k,v in cta_plan.items() if k not in ('logo','icon','brand_art')}}
    finally:
