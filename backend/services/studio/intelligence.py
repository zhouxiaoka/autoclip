...
def make_drafts(events, prefs):
    ...
    if prefs.goal == 'promo':
        ...
        return [Draft(id=uuid.uuid4().hex, title=h['title'], hook=h['hook'], scenes=[scene], language=prefs.language, aspect=prefs.aspect, layout='crop' if prefs.aspect == 'portrait' else 'fit', title_style='impact', subtitles=False, origin='visual-promo').model_dump() for h in hooks]
