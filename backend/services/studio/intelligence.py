...
def make_drafts(events, prefs):
    ...
    if prefs.goal == 'promo':
        ...
        return [Draft(id=uuid.uuid4().hex, title=h['title'], hook=h['hook'], scenes=[scene], language=prefs.language, aspect=prefs.aspect, subtitles=False, origin='visual-promo').model_dump() for h in hooks]
    return [Draft(id=uuid.uuid4().hex, title=e.label, scenes=[e], language=prefs.language, aspect=prefs.aspect, subtitles=False, origin='visual-highlight').model_dump() for e in events]
