...
| モデルの選択 | Qwen、OpenAI、Gemini、Claude、DeepSeek、および Ollama / LM Studio のローカルモデルに対応します（API キーは自分で用意）。 |
...
| 方式 | 設定 |
| --- | --- |
| クラウドモデル | 設定で Qwen、OpenAI、Gemini、Claude または DeepSeek を選び、API キーを入力します。互換インターフェースでは Base URL を指定できます。 |
