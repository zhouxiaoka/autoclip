...
AutoClip 本体は無料で、MIT ライセンスのオープンソースです。クラウドモデルは各社の利用料金がかかり、自分の API キーが必要です。Ollama / LM Studio はクラウドのキー不要ですが、モデルと対応するハードウェアが必要です。
...
ローカルの編集処理はお使いの端末で行います。クラウドモデルには字幕テキストを送信します。公開・アップロード機能を使った場合は、動画を指定先に送信します。利用統計やエラー報告はバージョンと設定によるため、プライバシー説明をご確認ください。
